...
export default function Page() {
  ...
  // 添加拖拽相关状态
  const [position, setPosition] = useState({ x: 0, y: -150 }); // 初始位置设在上方
  const [editPosition, setEditPosition] = useState({ x: 0, y: -150 }); // 编辑弹窗位置
  ...
  // 处理新文章表单输入变化
  const handleNewArticleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  // 处理编辑文章表单输入变化
  const handleEditArticleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    if (editingArticle) {
      setEditingArticle({
        ...editingArticle,
    ...
    <div className="p-5 max-w-7xl mx-auto relative">
      ...
      {!isLoading ? (
        <table className="w-full border-collapse mb-5">
          <thead>
            <tr>
              <th className="p-3 text-left border-b border-gray-100 bg-gray-50 font-medium">标题</th>
              <th className="p-3 text-left border-b border-gray-100 bg-gray-50 font-medium">内容</th>
              <th className="p-3 text-left border-b border-gray-100 bg-gray-50 font-medium">操作</th>
            </tr>
          </thead>
          <tbody>
            {articles.length > 0 ? (
              articles.map((article) => (
                <tr key={article.id}>
                  <td className="p-3 text-left border-b border-gray-100">{article.title}</td>
                  <td className="p-3 text-left border-b border-gray-100">{article.content}</td>
              ...
              <tr>
                <td colSpan={3} className="p-3 text-center border-b border-gray-100">
                  没有找到文章
                </td>
              </tr>
            )}
          ...
          {Array.from({ length: Math.ceil(total / pageSize) }, (_, i) => (
            <button
              ...
              className={`min-w-[32px] h-8 px-1.5 border bg-white rounded flex items-center justify-center hover:border-blue-500 hover:text-blue-500 ${
                pageNum === i + 1 ? "border-blue-500 text-blue-500 font-medium" : "border-gray-300"
              }`}
            >
              {i + 1}
            </button>
